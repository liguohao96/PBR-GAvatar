// COPY from https://github.com/NVlabs/nvdiffrecmc/blob/main/render/optixutils/c_src/optix_wrapper.cpp
...
void createPipeline(const OptixDeviceContext context, const std::string& path, const std::string& cuda_path, 
                const std::string& kernel_name, const std::string& kernel_file, OptixModule* module, OptixPipeline* pipeline, OptixShaderBindingTable& sbt)
{
    char log[2048];
    OptixPipelineCompileOptions pipeline_compile_options = {};
    {
        ...
        std::string shaderFile = path + "/" + kernel_name + "/" + kernel_file;
        std::string includeDir = path + "/" + kernel_name;
    ...
    //
    {
        ...
        OptixProgramGroup program_groups[] = { raygen_prog_group, miss_prog_group, hit_prog_group };
    ...
    //
    {
        ...
        // maybe check this out: https://www.willusher.io/graphics/2019/11/20/the-sbt-three-ways/
        CUdeviceptr hit_record;
        size_t      hit_record_size = sizeof( SbtRecord );
        CUDA_CHECK( cudaMalloc( reinterpret_cast<void**>( &hit_record ), hit_record_size ) );
        SbtRecord hit_sbt;
        OPTIX_CHECK( optixSbtRecordPackHeader( hit_prog_group, &hit_sbt ) );
        CUDA_CHECK( cudaMemcpy(
                    reinterpret_cast<void*>( hit_record ),
                    &hit_sbt,
                    hit_record_size,
                    cudaMemcpyHostToDevice
                    ) );
...
OptiXStateWrapper::OptiXStateWrapper(const std::string& path, const std::string& cuda_path)
{
    ...
    pState->fwd_mod = nullptr;
    pState->fwd_ppl = nullptr;
    pState->fwd_sbt = {};
    createPipeline(pState->context, path, cuda_path, "gaussiantracing", "forward.cu",  &pState->fwd_mod, &pState->fwd_ppl, pState->fwd_sbt);

    pState->bwd_mod = nullptr;
    pState->bwd_ppl = nullptr;
    pState->bwd_sbt = {};
    createPipeline(pState->context, path, cuda_path, "gaussiantracing", "backward.cu", &pState->bwd_mod, &pState->bwd_ppl, pState->bwd_sbt);

    pState->shd_mod = nullptr;
    pState->shd_ppl = nullptr;
    pState->shd_sbt = {};
    createPipeline(pState->context, path, cuda_path, "envsampling", "kernel.cu", &pState->shd_mod, &pState->shd_ppl, pState->shd_sbt);
...
OptiXStateWrapper::~OptiXStateWrapper(void)
{
    ...
    OPTIX_CHECK( optixPipelineDestroy( pState->fwd_ppl ) );
    CUDA_CHECK( cudaFree( reinterpret_cast<void*>( pState->fwd_sbt.raygenRecord       ) ) );
    CUDA_CHECK( cudaFree( reinterpret_cast<void*>( pState->fwd_sbt.missRecordBase     ) ) );
    CUDA_CHECK( cudaFree( reinterpret_cast<void*>( pState->fwd_sbt.hitgroupRecordBase ) ) );
    OPTIX_CHECK( optixModuleDestroy( pState->fwd_mod) );

    OPTIX_CHECK( optixPipelineDestroy( pState->bwd_ppl ) );
    CUDA_CHECK( cudaFree( reinterpret_cast<void*>( pState->bwd_sbt.raygenRecord       ) ) );
    CUDA_CHECK( cudaFree( reinterpret_cast<void*>( pState->bwd_sbt.missRecordBase     ) ) );
    CUDA_CHECK( cudaFree( reinterpret_cast<void*>( pState->bwd_sbt.hitgroupRecordBase ) ) );
    OPTIX_CHECK( optixModuleDestroy( pState->bwd_mod) );

    OPTIX_CHECK( optixPipelineDestroy( pState->shd_ppl ) );
    CUDA_CHECK( cudaFree( reinterpret_cast<void*>( pState->shd_sbt.raygenRecord       ) ) );
    CUDA_CHECK( cudaFree( reinterpret_cast<void*>( pState->shd_sbt.missRecordBase     ) ) );
    OPTIX_CHECK( optixModuleDestroy( pState->shd_mod) );
